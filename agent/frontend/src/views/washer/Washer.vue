      ...
      <v-stepper v-model="steps" alt-labels class="wt-stepper elevation-0">
        <v-stepper-header>
          <v-stepper-step
            ...
            :complete="steps > 1"
            step="1"
          >{{ $t('washer.step3.title') }}</v-stepper-step>
          <v-divider/>
          <v-stepper-step
            :class="$i18n.locale != 'ko' ? 'title' : $store.getters.isV2 ? 'headline': 'display-1'"
            :complete="steps === 2"
            step="2"
          >{{ $t('washer.step4.title') }}</v-stepper-step>
        </v-stepper-header>
        <v-stepper-items>
          <v-stepper-content step="1">
            <step3 :selected.sync="selected" :steps.sync="steps"/>
          ...
          <v-stepper-content step="2">
            <step4 :selected="selected" :course.sync="course" :dialog.sync="dialog"/>
            <v-layout justify-space-between>
              <v-flex xs3 class="text-xs-center">
                <v-btn
                  flat
        ...
        <v-card-text>
            ...
            <v-flex xs12 class="text-xs-center" mt-5>
              <p>
                <span class="display-2">
                  {{ $t('payment.desc1-1', { number:
                  selected !== null ?
                  $store.state.devices.washer[selected].controller_id : 0 }) }}
                </span>
                <br>
                <span class="display-2 wt-primary-font">{{ course.title }}</span>
                <span class="display-2" v-if="$i18n.locale === 'ko'">({{ course.description }})</span>
                <span
                  class="display-2"
                  v-else-if="$i18n.locale === 'en'"
                >({{ course.description_en }})</span>
                <span
                  class="display-2"
                  v-else-if="$i18n.locale === 'vi'"
                >({{ course.description_vn }})</span>
                <br>
                <span class="display-2">{{ $t('payment.desc2-1') }}</span>
                <br><br>
          ...
          >
            <v-flex xs8 class="text-xs-left">
              ...
              <span class="display-2 wt-primary-font">{{ course.amount }}</span>
            ...
            <v-flex xs6 class="text-xs-center" mt-4>
              <v-btn
                round
                class="display-2 wt-wave-bg wt-btn white--text py-5"
                :disabled="payment === 'card' && paymentPrice < course.amount"
          ...
          >
            <v-flex xs8 class="text-xs-left">
              ...
              <span class="display-1 wt-primary-font">{{ course.amount }}</span>
          ...
          <v-layout wrap justify-center align-center v-else-if="paymentSteps === 7" mt-5>
            <v-flex xs8 class="text-xs-center" mt-3>
              <span class="display-1">{{ $t('payment.washser-success') }}</span>
            </v-flex>
            <v-flex xs8 class="text-xs-center" mt-3>
              <span class="display-1">{{ $t('payment.close') }}</span>
            </v-flex>
...
<script>
import Step3 from '@/views/washer/steps/Step3'
import Step4 from '@/views/washer/steps/Step4'
  ...
  methods: {
    canPayment () {
      ...
      if (this.$store.state.agency.use_point !== 0 &&
        ...
        if (this.$store.state.user.point + this.$store.state.user.money >= this.course.amount) {
          return true
        }
      } else {
        if (this.$store.state.user.money >= this.course.amount) {
          return true
        }
      }
    ...
    trigger () {
      ...
      let deviceId = this.$store.state.devices.washer[this.selected].controller_id
        ...
        .then((res) => {
          if (this.$store.state.user.hasOwnProperty('phone')) {
            ...
            if (this.$store.state.user.point < this.$store.state.agency.use_point) {
              ...
              if (this.$store.state.user.point >= this.course.amount) {
                usePoint = this.course.amount
              } else {
                usePoint = this.$store.state.user.point
              }
            }

            if (this.$store.state.user.money >= this.course.amount - usePoint) {
              useMoney = this.course.amount - usePoint
            } else {
              this.paymentSteps = 6
              setTimeout(this.clearDialog, 2000)
              return
      ...
      if (this.payment === 'cash') {
        this.$axios.post('/cash/call')
          ...
          .catch((res) => {
            ...
            this.$socket.emit('cancel-payment', { client: 'true' })
          })
        this.interval = setInterval(function () {
          this.$axios.get('/cash/status')
            .then((res) => {
              if (res.data.money >= this.course.amount) {
                this.trigger()
                this.$socket.emit('cancel-payment', { client: 'true' })
              } else {
                if (res.data.money) {
