  ...
  <v-card class="mb-5 elevation-0" height="640px">
    <v-layout wrap justify-center align-center fill-height>
      <v-flex xs12 pa-1 class="text-xs-center" v-if="$i18n.locale === 'ko'">
        <span class="display-2">{{ $t('shoes-dryer.step2.desc1') }}&nbsp;</span>
        <span
          class="display-2 font-weight-bold wt-primary-font"
        >{{ $t('shoes-dryer.step2.desc2') }}</span>
        <span class="display-2">{{ $t('shoes-dryer.step2.desc3') }}</span>
        <!-- <span class="display-2 font-weight-bold wt-primary-font">{{ $t('shoes-dryer.step2.desc2') }}</span> -->
        <!-- <span class="display-2">{{ $t('shoes-dryer.step2.desc3') }}</span> -->
      </v-flex>
      <v-flex xs12 pa-1 class="text-xs-center" v-else>
        <span class="display-1">{{ $t('shoes-dryer.step2.desc1') }}&nbsp;</span>
        <span class="display-1">{{ $t('shoes-dryer.step2.desc2') }}&nbsp;</span>
        <span class="display-1">{{ $t('shoes-dryer.step2.desc3') }}</span>
      </v-flex>
      <v-flex v-for="(item, idx) in items" :key="item.id" xs4 pa-1 class="text-xs-center">
        <v-btn
          ...
          flat
          top
          @click="selectDryer(idx)"
        >
          <v-layout wrap ml-5>
            <v-flex xs12 ml-5>
              <span
                class="display-1"
              >{{ $t('shoes-dryer.step2.select', { number: item.controller_id }) }}</span>
            </v-flex>
          </v-layout>
        </v-btn>
      </v-flex>
...
export default {
  ...
  computed: {
    items () {
      return this.$store.state.devices['shoes-dryer']
    }
  },
  methods: {
    selectDryer (id) {
