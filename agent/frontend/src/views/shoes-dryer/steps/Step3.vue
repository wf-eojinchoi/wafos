  ...
  <v-card class="mb-5 elevation-0" height="640px">
    <v-layout wrap justify-center align-center fill-height>
      <v-flex xs12 pa-1 class="text-xs-center" v-if="$i18n.locale === 'ko'">
        <span class="display-2 font-weight-bold wt-primary-font">{{ $t('shoes-dryer.step3.desc1') }}</span>
        <span class="display-2">{{ $t('shoes-dryer.step3.desc2') }}</span>
      </v-flex>
      <v-flex xs12 pa-1 class="text-xs-center" v-else>
        <span class="display-1">{{ $t('shoes-dryer.step3.desc1') }}</span>
        <span class="display-1">{{ $t('shoes-dryer.step3.desc2') }}</span>
      </v-flex>
          ...
          <v-flex xs6 class="text-xs-center">
            <v-btn flat @click="minus()">
              <div>
                ...
                <br>
                <span class="display-1">{{ $t('shoes-dryer.step3.minus-time') }}</span>
              </div>
            </v-btn>
          </v-flex>
          <v-flex xs6 class="text-xs-center">
            <v-btn flat @click="plus()">
              <div>
                ...
                <br>
                <span class="display-1">{{ $t('shoes-dryer.step3.plus-time') }}</span>
              </div>
            </v-btn>
          </v-flex>
        </v-layout>
      ...
      <v-flex xs10 class="wt-dryer-timebox" mt-5>
        <v-layout wrap justify-space-between pa-4 v-if="$i18n.locale === 'ko'">
          <v-flex xs4 class="display-2 text-xs-center">{{ $t('shoes-dryer.step3.desc3') }}</v-flex>
        ...
        <v-layout wrap justify-space-between pa-4 v-else>
          <v-flex xs6 class="display-1 text-xs-center">{{ $t('shoes-dryer.step3.desc3') }}</v-flex>
...
export default {
  ...
  watch: {
    selected (val) {
      if (val !== null) {
        let dryer = this.$store.state.devices['shoes-dryer'][val]
  ...
  mounted () {
    if (this.selected) {
      let dryer = this.$store.state.devices['shoes-dryer'][this.selected]
      this.unit = dryer.min_etc_coin
      this.unitPrice = dryer.min_coin
      this.maxPrice = dryer.max_coin
      this.minPrice = dryer.current_coin
