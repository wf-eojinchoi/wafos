      ...
      <v-stepper v-model="steps" alt-labels class="wt-stepper elevation-0">
        <v-stepper-header>
          <v-stepper-step
            ...
            :complete="true"
            step="1"
          >{{ $t('airconditioner.step1.title') }}</v-stepper-step>
        </v-stepper-header>
        <v-stepper-items>
          <v-stepper-content step="1">
            <step1 :minutes.sync="minutes" :price.sync="price"/>
        ...
        <v-card-text>
          ...
          <v-layout wrap justify-center align-center v-if="paymentSteps === 1" mt-5>
            <v-flex xs12 class="text-xs-center" mt-5>
              <p>
                <span class="display-2">{{ $t('payment.airconditioner') }}</span>
          ...
          <v-layout wrap justify-center align-center v-else-if="paymentSteps === 4" mt-5>
            ...
            <v-flex xs6 class="text-xs-center" mt-4>
              <v-btn
                ...
                @click.once="requestPayment(); paymentSteps = 5"
              >
          ...
          <v-layout wrap justify-center align-center v-else-if="paymentSteps === 7" mt-5>
            <v-flex xs8 class="text-xs-center" mt-3>
              <span class="display-1">{{ $t('payment.air-success') }}</span>
            </v-flex>
            <v-flex xs8 class="text-xs-center" mt-3>
              <span class="display-1">{{ $t('payment.close') }}</span>
            </v-flex>
...
<script>
import Step1 from '@/views/airconditioner/steps/Step1'
...
export default {
  ...
  methods: {
    ...
    trigger () {
      ...
      let deviceId = this.$store.state.devices.airconditioner[0].controller_id
