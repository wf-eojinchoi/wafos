  ...
  <v-content>
    <v-layout row wrap>
      <v-toolbar>
        ...
        <v-toolbar-title>일별 매출 현황</v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    loadPayList () {
      ...
      this.loading = true
      console.log(this.date, this.$store.state.adminAgency.wash)
      this.$store.dispatch('PaymentDayList', {
        page: this.pagination.page,
        agency_id: this.$store.state.adminAgency.wash,
        year: this.date?this.date.substr(0, 4):null,
  ...
  mounted () {
    this.$store.dispatch('updateTitle', '일별 매출 관리')
    this.loadPayList()
  },
  watch: {
    pagination: {
